...
void Log::PrintToFile(const char *str)
{
  ...
  if (hFile == -1)
    hFile = _open(filename, A_ReadWrite + A_Create + A_Append + A_BIN, P_WRITE, &io_error);
  if(!io_error)
  {
    _write(hFile, str, strlen(str), &io_error);
    _write(hFile, "\r\n", 2, &io_error);
  }
}
