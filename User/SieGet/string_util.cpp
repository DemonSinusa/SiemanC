...
*/
char * Get_Param_Value(char * str, const char * req, int cut_quotes, int separator)
{
  ...
  if (!req_displace || value[0] != '=') return 0;
  value += 1;
...
void remove_bad_chars(char *s)
{
  ...
  while((c=*s))
  {
    for (unsigned int i=0; i<(sizeof(badchars)/sizeof(char)); i++)
    {
      if (c==badchars[i])
      {
        *s='_';
