...
#pragma pack()
  ...
  if ((buffer=malloc(65536)))
  {
    ...
    if ((fin=_open(s_ab_main, A_ReadOnly+A_BIN, P_READ, &ul))!=-1)
    {
#ifdef ELKA
      if (_read(fin,&ABmain,sizeof(ABmain),&ul)>=194)
#else
	if (_read(fin,&ABmain,sizeof(ABmain),&ul)==sizeof(ABmain))
#endif
	{
	  _close(fin,&ul);
	  do
...
#endif
		if ((fin=_open(recname,A_ReadOnly+A_BIN,P_READ,&ul))!=-1)
		{
		  zeromem(&ur,sizeof(AB_UNPRES));
		  fsz=_lseek(fin,0,S_END,&ul,&ul);
		  _lseek(fin,0,S_SET,&ul,&ul);
		  _read(fin,buffer,fsz,&ul);
		  _close(fin,&ul);
#ifdef NEWSGOLD
