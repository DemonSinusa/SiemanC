...
//void setColor(int r, int g, int b, int a, char* color);
int isElka();
...
void method0_2(MAIN_GUI_2 *data)
{
  ...
  DrawRectangle(0,isElka(),scr_w-1,scr_h-1,0,white,white);

  int fsize=GetFontYSIZE(font)+1;
  int column_height=scr_h-fsize-fsize-isElka();
  int column_width=scr_w/9;
  int start_column;
  int y_line;
  wsprintf(data->ws1,"%02X,%02X,%02X,%02X",data->r,data->g,data->b,data->a);

  DrawString(data->ws1,1,isElka()+1,scr_w-20,isElka()+fsize,font,1,black,NULL);
  fsize+=3;
  for (int i=0;i!=4;i++)
  {
    start_column=column_width+2*i*column_width;
    if (data->current_column==i)
      DrawRectangle(start_column-2,isElka()+fsize-2,start_column+column_width+2,isElka()+fsize+column_height+2,
                    0,black,white);

    DrawRectangle(start_column,isElka()+fsize,start_column+column_width,isElka()+fsize+column_height,
                  0,black,colors[i]);
    switch(i)
    {
    case 0:
      y_line=isElka()+fsize+column_height-(data->r*column_height)/0xFF;
      break;
    case 1:
      y_line=isElka()+fsize+column_height-(data->g*column_height)/0xFF;
      break;
    case 2:
      y_line=isElka()+fsize+column_height-(data->b*column_height)/0xFF;
      break;
    case 3:
      y_line=isElka()+fsize+column_height-(data->a*column_height)/0x64;
      break;
    }
    DrawLine(start_column,y_line,start_column+column_width,y_line,0,black);
  }
  setColor(data->r,data->g,data->b,data->a,data->testcolor);
  DrawRoundedFrame(scr_w-1-fsize,isElka()+1,scr_w-2,isElka()+fsize,2,2,0,black,data->testcolor);
}
...
void EditColors(char *color)
{
  ...
  patch_rect((RECT*)&Canvas_2,0,isElka(),ScreenW()-1,ScreenH()-1);
