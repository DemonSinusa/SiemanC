...
int _IsBadChar(short wch) {
	for(uint32_t cc = 0; cc < sizeof(badchars)/sizeof(short); cc++)
...
int fcopy(char* src, char* dst) {
	...
	int fe = fexist(dst);
	if (CONFIG_CONFIRM_REPLACE && fe) {
		int ret=MsgBoxYesNoWithParam(ind_pmt_exists, strrchr(dst,'\\')+1);
		if (ret != IDYES && ret != IDYES2ALL)
			return 1; // Все хорошо, если не стали перезаписывать
	}

	if(fe) {
		SetFileAttrib(dst, 0, &err);
		fsrm(dst, 0);
	}
...
int rmtree(char* path, int ip) {
	...
	while(itm && !progr_stop) {
		//incfname(GetFileName(itm->full));
		switch (itm->ftype) {
		case TYPE_COMMON_DIR:
			res &= _rmdir(itm->full, &err);
			break;
		case TYPE_COMMON_FILE:
			res &= _unlink(itm->full, &err);
			break;
		case TYPE_ZIP_DIR:
		case TYPE_ZIP_FILE:
		case TYPE_7Z_DIR:
...
int fsmv(char* src, char* dst) {
	...
	if (src[0] == dst[0]) {
		if (stricmp(src, dst) != 0) // Только не совпадает
			res = fmove(src, dst, &err);

		incprogr(GetFilesCnt(dst));
	} else {
		if (fscp(src, dst, 1) && !progr_stop)
			res = fsrm(src, 0);
	}
	return res;
}
