...
void DoDiskAccess(unsigned int flag)
{
  ...
  if ((f=stk_fhandle)==-1)
  {
    stk_fhandle=f=_open(stkfile,A_ReadWrite+A_BIN,P_READ+P_WRITE,&ul); //Файл верхнего стека
  ...
  if (disk_access&LOAD_UP)
  {
    ...
    bfree[b]=0;
    seekpos=STKSZ50*b;
    if (_lseek(f,seekpos,0,&ul,&ul)!=seekpos) DiskErrorMsg(2);
    if (_read(f,ustk,STKSZ50,&ul)!=STKSZ50) DiskErrorMsg(0);
    usp+=STKSZ50;
  }
  if (disk_access&LOAD_DOWN)
  {
    ...
    seekpos=STKSZ50*b;
    if (_lseek(f,seekpos,0,&ul,&ul)!=seekpos) DiskErrorMsg(2);
    if (_read(f,dstk+STKSZ50,STKSZ50,&ul)!=STKSZ50) DiskErrorMsg(0);
    dsp-=STKSZ50;
  }
  if (disk_access&SAVE_UP)
  {
    ...
    seekpos=STKSZ50*b;
    if (_lseek(f,seekpos,0,&ul,&ul)!=seekpos) DiskErrorMsg(2);
    if (_write(f,ustk,STKSZ50,&ul)!=STKSZ50) DiskErrorMsg(1);
  ...
  if (disk_access&SAVE_DOWN)
  {
    ...
    seekpos=STKSZ50*b;
    if (_lseek(f,seekpos,0,&ul,&ul)!=seekpos) DiskErrorMsg(2);
    if (_write(f,dstk+STKSZ50,STKSZ50,&ul)!=STKSZ50) DiskErrorMsg(1);
...
unsigned int SearchHistory(void)
{
  ...
  if ((fin=_open(historyfile,A_ReadOnly+A_BIN,P_READ,&ul))!=-1)
  {
    for(;;)
    {
      i=_read(fin,&HISTORY,sizeof(HISTORY),&ul);
...
void SaveHistory(void)
{
  ...
  if ((f=_open(historyfile,A_ReadWrite+A_BIN,P_READ+P_WRITE,&ul))==-1)
    f=_open(historyfile,A_ReadWrite+A_BIN+A_Create,P_READ+P_WRITE,&ul);
  if (f!=-1)
  {
    _lseek(f,sizeof(HISTORY)*history_pos,0,&ul,&ul);
    _write(f,&HISTORY,sizeof(HISTORY),&ul);
    _close(f,&ul);
  }
  /*  if (terminated)
  {
  ...
  if (FindFirstFile(&de,path,&err))
  {
    do
    {
      if (!(de.file_attr&FA_DIRECTORY))
      {
        ...
        if ((fin=_open(fname,A_ReadOnly+A_BIN,P_READ,&ul))!=-1)
        {
          _read(fin,&tmp,1,&ul);
          _read(fin,&size,1,&ul);
          _read(fin,fn_font,16,&ul);
          _close(fin,&ul);
...
void LoadFont(int flag)
{
  ...
  if ((fin=_open(cur->file,A_ReadOnly+A_BIN,P_READ,&ul))!=-1)
  {
    ...
    for(i = 0; i < 256; i++)
    {
      _read(fin,&chars_width_src[i],1,&ul);
      _read(fin,font+i*FW*FH,FW*FH,&ul);
    }
    _close(fin,&ul);
  }
  chars_width_src[1] = 1;
...
unsigned int FL_loader(int fin, unsigned int p)
{
  ...
  if (p>=STKSZ50) //Если указатель перешел за границу половины буфера
  {
    ...
    i=_read(fin,dstk+STKSZ50,STKSZ50,&ul); //Следующие пол-буфера
...
unsigned int FL_saver(int fs, unsigned int p)
{
  ...
  if (p>=STKSZ50)
  {
    ...
    ubat[++u_disk]=b;
    seekpos=STKSZ50*b;
    if (_lseek(fs,seekpos,0,&ul,&ul)!=seekpos) DiskErrorMsg(2);
    if (_write(fs,ustk,STKSZ50,&ul)!=STKSZ50) DiskErrorMsg(1);
    p-=STKSZ50;
...
//  GetFontList();
  ...
  fs=_open(stkfile,A_Create+A_ReadWrite+A_BIN,P_READ+P_WRITE,&ul); //Файл верхнего стека
  if (fs==-1) DiskErrorMsg(3);
  if ((fin=_open(filename,A_ReadOnly+A_BIN,P_READ,&ul))!=-1)
  {
    switch(fmt&0x7F)
    {
    case 1:
