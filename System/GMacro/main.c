...
unsigned int GetRand(unsigned int MaxRND) {
...
// Работа с файлами стандартна:
	handle=_open(path,A_ReadOnly+A_BIN,P_READ,&err);
	if(handle==-1) return 0;

	len=_lseek(handle,8*1024*1024,0,&err,&i);// ->size of file if fails
	_lseek(handle,0,0,&err,&i);
	mem=(char*)malloc(len+1);
	if(mem!=0) {
		sz=_read(handle,mem,len,&err);
...
void Step() {
	...
	case ST_NORM: {
		...
		case '|': {
			...
			if(ch==-1) {
				s=ST_ERROR;
				sprintf(ebu,"Unk character %s",xs(po));
				break;
			}
			KeyUp(ch);
		}
