...
int IsFieldCorrect(void *data, int ec_index)
{
  ...
  if ((ec_index>1)&&(ec_index&1))
  {
    ...
    wstrcpy(ws1,ec.pWS);
    ws_2str(ws1,ss,15);
    #warning "Kak bez libc i +++ neznaii.."
    __set_errno(0);
    switch(hp->type)
    {
    case CFG_UINT:
      vui=strtoul(ss,0,10);
      if (vui<hp->min || vui>hp->max || !ws1->wsbody[0] || (err=*__errno_location())==ERANGE)
      {
    ...
    case CFG_INT:
      vi=strtol(ss,0,10);
      if (vi<(int)hp->min || vi>(int)hp->max || !ws1->wsbody[0] || (err=*__errno_location())==ERANGE)
      {
...
int LoadCfg(char *cfgname)
{
  ...
  {
    if ((f=_open(cfgname,A_ReadOnly+A_BIN,P_READ,&err))!=-1)
    {
      size_cfg=fstat.size;
      if (size_cfg<=0)
      {
      ...
      {
        cfg=(char *)malloc((size_cfg+3)&(~3));
        if (_read(f,cfg,size_cfg,&err)!=size_cfg)
        {
  ...
  if (FindFirstFile(&de,str,&err))
  {
    do
    {
      if (!(de.file_attr&FA_DIRECTORY))
      {
        ...
        sb=(SEL_BCFG *)malloc(sizeof(SEL_BCFG));
...
{
  unsigned int ul;
  int f=_open(cfg_name,A_ReadWrite+A_Create+A_Truncate+A_BIN,P_READ+P_WRITE,&ul);
  if (f!=-1)
  {
    _write(f,cfg,size_cfg,&ul);
    _close(f,&ul);
  }
