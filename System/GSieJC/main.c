...
void _start_vibra(GBSTMR *t)
{
  void _stop_vibra(GBSTMR *);
  if(Is_Vibra_Enabled)
  {
...
int ReadDefSettings(char *elfpath)
{
  ...
  if ((f=_open(elfpath,A_ReadOnly+A_BIN,P_READ,&err))!=-1)
  {
    int rlen = _read(f,&def_set,sizeof(DEF_SETTINGS),&err);
    int end=_lseek(f,0,S_END,&err,&err);
    _close(f,&err);
...
int WriteDefSettings(char *elfpath)
{
  ...
  if ((f=_open(elfpath,A_WriteOnly+A_BIN+A_Create+A_Truncate,P_WRITE,&err))==-1) return 0;
    ...
    _write(f, &def_set, sizeof(DEF_SETTINGS), &err);
    _close(f, &err);
    return 1;
}
