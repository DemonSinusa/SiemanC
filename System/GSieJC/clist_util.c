  ...
  while(ClEx)
  {
    if(ClEx->ResourceCount)
    {
      ...
      while(resEx)
      {
        ...
        if(( (uint32_t)i > (Active_page-1)*N_cont_disp) && ((Display_Offline  |  resEx->status!=PRESENCE_OFFLINE | resEx->has_unread_msg) && Is_Right_Vis_Mode))
        {
          if(i == CursorPos)
          {
            lineColor=CURSOR;
...
unsigned int CList_GetNumberOfUsers_Visible()
{
  ...
  while(ClEx)
  {
    while(ClEx && (CList_isGroup(ClEx) || CList_isMUC(ClEx)) && ClEx->IsVisible == 0) //Перескакиваем через свернутые группы и конференции, иначе промахнемся
    {
...
void MoveCursorTo(TRESOURCE* NewResEx)
{
  ...
  while(ClEx)
  {
    while((CList_isGroup(ClEx) || CList_isMUC(ClEx)) && ClEx->IsVisible == 0) //Перескакиваем через свернутые группы и конференции, иначе промахнемся
    {
      char c_group = ClEx->group;
      if (c_group & 0x80) // Пропускаем конференции
      {
