...
uint32_t CurrentMessage = 0;
uint32_t CurrentMessage_Lines = 0;
uint32_t lines_on_page;
uint32_t MaxPages=1;
...
void Calc_Pages_Data()
{
  ...
  if((uint32_t)lines_on_page * MaxPages < DispMessList_Count) MaxPages++;
}
...
void mGUI_onRedraw(GUI *data)
{
  ...
  int Y_pos = 0; // Стартовая позиция текущей строки
  while(ml)
  {
      ...
      case LEFT_BUTTON: //страница вверх
        {
          if (CurrentPage>1)
          {
            ...
            for(;;)
            {
              ...
              CurrentMessage--;
              Calc_Pages_Data_1();
              if (CurrentPage +1 == (uint32_t)cp || CurrentPage < 1)
                break;
            }
          }
      ...
      case RIGHT_BUTTON:
        {
          if (CurrentPage<MaxPages)
          {
            ...
            for(;;)
            {
              ...
              CurrentMessage++;
              Calc_Pages_Data_2();
              if (CurrentPage-1 == (uint32_t)cp || CurrentPage>=MaxPages)
                break;
            }
          }
          REDRAW();
...
void ParseMessagesIntoList(TRESOURCE* ContEx)
{
  ...
  while(MessEx)
  {
    if((uint32_t)parsed_counter >= OLD_MessList_Count)
    {
	...
	else
	{
	  ...
	  if (line_width > screen_width)
	  {
	    ...
	    goto L_ADD; //Добавить строку
          ...
          if (Is_Smiles_Enabled && SmilesImgList)
          {
            if (symb >= (uint32_t)FIRST_UCS2_BITMAP && symb <= (uint32_t)FIRST_UCS2_BITMAP + smiles_loaded) // Если код смайла
