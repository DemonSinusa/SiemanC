...
void Log(char* module, char* logstr)
{
  ...
  if(!io_error)
  {
    _write(hFile, buffer, buf_len-1, &io_error);
...
void Log_XMLStream(char* logstr, int size)
{
  ...
  if(!io_error)
  {
    _write(hFile, logstr, size, &io_error);
    _close(hFile, &io_error);
  }
  else
  {
...
void _add2history(HIST_RECORD_INFO *info)
{
  ...
  hFile = _open(filename_utf8,A_ReadWrite + A_Append + A_BIN,P_READ+P_WRITE, &io_error);
  
  if(io_error==2) // нет файла
  {
    hFile = _open(filename_utf8,A_ReadWrite+A_Create+ A_BIN,P_READ+P_WRITE, &io_error);
  }
  if(!io_error)
  {
    _write(hFile, info->buffer, info->txt_len-1, &io_error);
    _close(hFile, &io_error);
  }
  else
  {
