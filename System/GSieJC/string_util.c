...
OUT: out_ofs - смещение в строке, откуда начинаются необработанные данные
    <return> - спецсимвол
*/
char GetSpecialSym(char *mask_begin, uint32_t *out_ofs)
{
  int i=0;
  int replen;
  char rep_ex[10];
...
char * Replace_Special_Syms(char * unrep_str)
{
  ...
  for(uint32_t j=0;j<unrep_len;j++)
  {
    tmp = *(unrep_str+j);
...
char * Mask_Special_Syms(const char * unrep_str)
{
  ...
  for(uint32_t i=0; i<unrep_len; i++)
  {
...
char * ANSI2UTF8(const char * ansi_str, unsigned int maxlen)
{
  ...
  ws_2utf8(ws_str, utf8_str, (int*)&utf8_len, maxlen * 2 + 1);
  FreeWS(ws_str);
...
void remove_bad_chars(char *s)
{
  ...
  while((c=*s))
  {
    for (uint32_t i=0; i<(sizeof(badchars)/sizeof(char)); i++)
    {
      if (c==badchars[i])
      {
        *s='_';
