...
char *successed_extfile="?:\\ZBin\\etc\\extension.cfg";
...
int ParseExtCfg(const char *fname)  // Разобьем файл на строки
{
  ...
  if (GetFileStats(fname,&fstat,&err)<0){
  		 return (-1);
  }
  if ((f=_open(fname,A_ReadOnly|A_BIN,P_READ,&err))==-1){
  		 return (-1);
  }
  p=buf=malloc(fstat.size+1);
  buf[_read(f,buf,fstat.size,&err)]=0;
  _close(f,&err);
  fl=p;
  for(;;)
  {
  ...
  if (msg->keys==0xFFF)
  {
    ...
    *s=0;
    ...
    if (is_any_change)
    {
      ...
      char r='\r';
      void *gui;
      if ((f=_open(successed_extfile,A_WriteOnly|A_BIN|A_Truncate|A_Create,P_WRITE,&err))!=-1)
      {
        for (int i=0; i<lines_n; i++)
        {
          _write(f,lines[i],strlen(lines[i]),&err);
          _write(f,&r,1,&err);
        }
        _close(f,&err);
      }
...
HEADER_DESC new_or_editext_hdr={0,0,0,0,NULL,(int)"Edit:",LGP_NULL};
...
void main_menu_iconhndl(void *data, int curitem, void *unk)
{
  ...
  void *item=AllocMenuItem(data);
  {
    ...
    if ((index=es[curitem].ext)==-1)
      wsprintf(ws,"%t","UNI_EXT");
    else
      wsprintf(ws,"%t",lines[index]);
  }
...
void maincsm_oncreate(CSM_RAM *data)
{
  ...
  successed_extfile[0]='4';
  if ((result=ParseExtCfg(successed_extfile))<0)
  	successed_extfile[0]='0';
    result=ParseExtCfg(successed_extfile);
  if (result>=0)
  {
    ParseLines();
...
{
  wsprintf((WSHDR *)(&MAINCSM.maincsm_name),"GExtMan");
}
