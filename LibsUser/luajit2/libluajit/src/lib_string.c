...
LJLIB_ASM(string_at)		LJLIB_REC()
{
  GCstr *s = lj_lib_checkstr(L, 1);
  int32_t pos = lj_lib_checkint(L, 2);
  if((int32_t)s->len < pos-1) {
    lj_err_callermsg(L, "Index value bigger than string len");
    return 0;
  }

  int test;
  char *buf = (char *)&test;
  //char *buf = lj_str_needbuf(L, &G(L)->tmpbuf, (size_t)2);

  buf[0] = strdata(s)[pos-1];
  buf[1] = 0;
  setstrV(L, L->base-1, lj_str_new(L, buf, (size_t)1));
  return FFH_RES(1);
}
