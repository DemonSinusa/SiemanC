...
static void AutoSwitcher(void)
{
	...
	else
	{
		...
		GBS_StartTimerProc(&tmr_auto_switch, TMR_SEC, (void*)AutoSwitcher);
	}
...
static void Play(void)
{
	...
	{
		GBS_StartTimerProc(&tmr_start_playing, TMR_SEC/4, (void*)Play);
	}
}
...
void APlayer_NextTrack(void)
{
	...
	if (play_status != APLAYER_PLAY)
	{
		if (APlayer_GetTotalTracks())
		{
			ChangeTrack(WAY_NEXT);
			InitTrackData();
			GBS_StartTimerProc(&tmr_play_after, tmrdiv3, (void*)BeforeChangeTrack);
		}
	}
}
...
void APlayer_Forward(unsigned int inc_s)
{
	if (track_dur_ms)
	{
		...
		{
			IPC_SendMessage(&ipc, APD_IPC_NAME, "libapd", APD_IPC_UPDATE_POS, 0);
			GBS_StartTimerProc(&tmr_play_after,tmrdiv3, (void*)After);
		}
	}
}
